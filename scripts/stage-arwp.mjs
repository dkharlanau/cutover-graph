...
if (html.includes(`id="${marker}"`)) html = html.replace(/<link\s[^>]*id="arwp-profile-discovery"[^>]*>/i, link);
else { if (!/<\/head>/i.test(html)) throw new Error("Public homepage has no closing head"); html = html.replace(/<\/head>/i, `${link}\n</head>`); }
...
const favicon = publicUrl.hostname === "dkharlanau.github.io" ? new URL("/assets/favicons/icon-192.png", hostnameRoot).href : new URL("/favicon.png", hostnameRoot).href;
const escapeAttr = (value) => String(value).replaceAll("&", "&amp;").replaceAll('"', "&quot;").replaceAll("<", "&lt;").replaceAll(">", "&gt;");
const ensureMeta = (attribute, key, value) => { const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); const pattern = new RegExp(`<meta\\s+${attribute}=["']${escapedKey}["'][^>]*>`, "i"); const tag = `<meta ${attribute}="${key}" content="${escapeAttr(value)}" data-arwp-search-appearance="true">`; html = pattern.test(html) ? html.replace(pattern, tag) : html.replace(/<\/head>/i, `${tag}\n</head>`); };
...
if (/<link\s+[^>]*data-arwp-search-appearance=["']true["'][^>]*rel=["']icon["'][^>]*>/i.test(html)) html = html.replace(/<link\s+[^>]*data-arwp-search-appearance=["']true["'][^>]*rel=["']icon["'][^>]*>/i, faviconTag); else html = html.replace(/<\/head>/i, `${faviconTag}\n</head>`);
